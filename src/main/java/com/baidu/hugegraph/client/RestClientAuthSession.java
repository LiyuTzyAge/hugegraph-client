...
public abstract class RestClientAuthSession
{
    ...
    public RestClientAuthSession(String url, int timeout) {
        this(url, new ConfigBuilder().config(timeout).build());
    }


    public RestClientAuthSession(String url, int timeout, int maxTotal, int maxPerRoute) {
        this(url, new ConfigBuilder().config(timeout)
                .config(maxTotal, maxPerRoute)
                .build());
    }

    public RestClientAuthSession(String url, String user, String password, int timeout) {
        this(url, new ConfigBuilder().config(timeout)
    ...
    public RestClientAuthSession(String url, ClientConfig config) {
    ...
    private String getBasicAuthMsg()
    {
        UserAccessContext.User user = UserAccessContext.getUser();
        if (null == user) {
            throw new ClientException("user Access voucher is null");
        }
        String msg = Base64.encodeAsString(user.getUser() + ":" + user.getPwd());
        return "Basic " + msg;
    }
    ...
    {
        return UserAccessContext.isAuth();
    }

    private MultivaluedMap<String, Object> appendAuthHeader(MultivaluedMap<String, Object> headers)
    {
        headers = null == headers ? new MultivaluedHashMap<>() : headers;
        headers.putSingle("Authorization", getBasicAuthMsg());
        return headers;
    }

    private Builder appendAuthHeader(Builder request)
    {
        return request.headers(appendAuthHeader((MultivaluedMap<String, Object>) null));
    }
    ...
    public RestResult post(String path, Object object,
                            MultivaluedMap<String, Object> headers,
                            Map<String, Object> params) {
        //add header for http Basic auth
        if (isAuth()) {
    ...
    public RestResult get(String path) {
        Response response = this.request(() -> {
            Builder request = this.target.path(path).request();
            if (isAuth()) {
                return appendAuthHeader(request).get();
            }
            return request.get();
        });
    ...
    public RestResult get(String path, Map<String, Object> params) {
        ...
        Response response = this.request(() -> {
            Builder request = target.get().path(path).request();
            if (isAuth()) {
                return appendAuthHeader(request).get();
            }
            return request.get();
    ...
    public RestResult get(String path, String id) {
        Response response = this.request(() -> {
            Builder request = this.target.path(path).path(encode(id)).request();
            if (isAuth()) {
                return appendAuthHeader(request).get();
            }
            return request.get();
    ...
    public RestResult delete(String path, Map<String, Object> params) {
        ...
        Response response = this.request(() -> {
            Builder request = target.get().path(path).request();
            if (isAuth()) {
                return appendAuthHeader(request).delete();
            }
